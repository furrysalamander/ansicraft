...
use minecraft_terminal_viewer::{config, minecraft, render};
use crate::queueing::{self, ResourceAllocator, ResourcePool};

use anyhow;
use rand_core::OsRng;
use russh::{self, keys::{ssh_key::{self}, PublicKeyBase64}, server::Server};
use tokio::sync::mpsc;
...
pub struct MinecraftClientSession {
    ...
    terminal_size: Arc<Mutex<config::TerminalSize>>, // Store terminal size for resize events
...
impl Server for MinecraftSshServer {
    ...
    fn new_client(&mut self, _peer_addr: Option<std::net::SocketAddr>) -> Self::Handler {
        ...
        MinecraftClientSession {
            ...
            terminal_size: Arc::new(Mutex::new(config::TerminalSize {
                target_width: 10,
                target_height: 10,
            })),
            input_channel_tx,
...
impl MinecraftClientSession {
    ...
    fn set_terminal_size(&mut self, width: u32) -> anyhow::Result<()> {
        ...
        size.target_height = render::get_height_from_width(width as usize);
        Ok(())
    }
